...
//	printf("Starting clock\n");
	...
	cudaErrorCheck("Memory Copy Error on 114");
	
	cudaMemcpy(d_data1, h_data1, nx * sizeof(cufftComplex), cudaMemcpyHostToDevice);
	cudaErrorCheck("Memory Copy Error on 116");
...
//	dim3 dimGrid((int)sqrt((float)batch) +1, (int)sqrt((float)batch) +1);
	...
	dimGrid.x = (int)sqrt((float)batch) +1 ;
	dimGrid.y = (int)sqrt((float)batch) +1 ;
//	dim3 dimBlk(nx);
